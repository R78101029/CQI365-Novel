...
除非它們是同一個褶痕在不同位置的投影。同一個摺疊面。在一個地方碰到了公元169年。在另一個地方碰到了1947年。同一個面。同一個基頻。不同的位置。
...
但那些數字之外——那些儀器能測量的東西之外——還有別的。那個運動攜帶了什麼。不是頻率。不是振幅。不是任何有單位的物理量。

是別的。

她沒有詞可以描述那個「別的」。她的語言裡沒有這個詞。也許任何語言裡都沒有。
