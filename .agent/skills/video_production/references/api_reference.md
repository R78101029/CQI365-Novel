...
## 小云雀（字節跳動的 Seedance 前端，目前未使用）

首發官方 Seedance 2.5 的創作平台。**是網頁產品不是 API**，塞不進 repo 內的管線。
值得知道的是它的一個能力：**從圖片生成 3D 白模**，用來預演角色動作、走位、
鏡頭軌跡與構圖——本質上是我們 animatic 那一關的加強版（我們是靜態圖排時間軸，
它是可調的立體預演）。

用它的前提是接受兩件事：工作流離開 repo（劇本／分鏡表不再是 git 裡的真實來源），
以及同一個 Seedance 模型對寫實真人臉大概率有同樣的限制。
真的需要精確控制走位時再考慮，一般鏡頭沒必要。
